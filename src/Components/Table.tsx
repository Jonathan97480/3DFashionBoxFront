...
export default function Table() {
    ...
    const SpecilaColumn = ({ keyColumn, row, index }: SpecialColumnProps) => {
        ...
        const iconMachine = (value: string | number | boolean | null) => {
            const emu_id = typeof value === 'number' ? value : parseInt(value as string, 10);
            switch (emu_id) {
                case 10:
                    return megaDrive;
                case 6:
        ...
        const iconCategory = (value: string | number | boolean | null) => {
            const category = typeof value === 'number' ? value : parseInt(value as string, 10);
            switch (category) {
                case 4:
                    return conbatGroupe;
                case 5:
        ...
        switch (keyColumn) {
            case 'emu_id':
                return <td key={keyColumn + index + row.pid}><img src={iconMachine(row[keyColumn as keyof GamesInterface])} alt="" width={50} /></td>;
            case 'category':
                return <td key={keyColumn + index + row.pid}><img src={iconCategory(row[keyColumn as keyof GamesInterface])} alt="" width={50} /></td>;
            case 'is_show':
                return <td key={keyColumn + index + row.pid}><img src={row[keyColumn as keyof GamesInterface] ? show : noShow} alt="" width={50}
                    onClick={() => handleSetIsShow(row)}
                /></td>;
            case 'is_favourite':
                return <td key={keyColumn + index + row.pid}><img src={row[keyColumn as keyof GamesInterface] ? fav : noFav} alt="" width={50}
                    onClick={() => handleSetIsFav(row)}
                /></td>;
            case 'action':
                return <td className="clickable" onClick={() => setAlert({
                    ...defaultAlertProps,
            ...
            default:
                return <td className="clickable" key={keyColumn + index + row.pid} >{row[keyColumn as keyof GamesInterface]}</td>;
        }
